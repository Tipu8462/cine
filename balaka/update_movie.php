...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update_movie")) {
  $updateSQL = sprintf("UPDATE movies SET category_id=%s, quality_id=%s, movie_name=%s, movie_cast=%s, movie_director=%s, movie_desc=%s, movie_keyword=%s, status=%s WHERE movie_id=%s",
...
mysql_select_db($database_db_connection, $db_connection);
$query_pro_cats = "SELECT * FROM categories";
...
mysql_select_db($database_db_connection, $db_connection);
$query_pro_brands = "SELECT * FROM quality";
...
mysql_select_db($database_db_connection, $db_connection);
$query_update_movie = sprintf("SELECT * FROM movies WHERE movie_id = %s", GetSQLValueString($colname_update_movie, "int"));
