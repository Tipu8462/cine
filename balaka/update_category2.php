...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "update_category")) {
  $updateSQL = sprintf("UPDATE categories2 SET category_name=%s WHERE category_id=%s",
  ...
  $updateGoTo = "add_categories2.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_db_connection, $db_connection);
$query_update_category = sprintf("SELECT * FROM categories2 WHERE category_id = %s", GetSQLValueString($colname_update_category, "int"));
